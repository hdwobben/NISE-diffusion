...
if [ -z "$REMOVE" ]; then 
  OUTPUT=$(./build/Release/bin/kubo_diff_int "$@" | tee /dev/tty | tail -1)
  eval "$python_interpreter" plot_integrand.py $OUTPUT;
else 
  ./build/Release/bin/kubo_diff_int "$@" -o out.tmp
  eval "$python_interpreter" plot_integrand.py out.tmp
fi