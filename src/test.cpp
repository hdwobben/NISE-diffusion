...
std::vector<double> loadDoubles(std::string const &fname)
{
    std::ifstream in(fname, std::ios::in | std::ios::binary);
    if (in)
    {
        ...
        in.seekg(0, std::ios::end);
        contents.resize(in.tellg() / sizeof(double));
        in.seekg(0, std::ios::beg);
        in.read(reinterpret_cast<char *>(contents.data()), contents.size() * sizeof(double) / sizeof(char));
        in.close();
...
void updateTrajectory(MatrixXd &Hf, double *rnd, Params const &p)
{
    ArrayXd r{p.N};
...
ArrayXd evolve(double *rnd, MatrixXd &H0, MatrixXd &Hf0,
               VectorXcd &c0, RowVectorXd &xxsq, Params const &p)
{
    ...
    for (int ti = 0; ti < p.nTimeSteps; ++ti)
    {
        ...
        updateTrajectory(Hf, rnd + (p.N * ti), p);
        H = H0 + Hf;
...
int main(int argc, char *argv[])
{
    ...
    std::vector<double> rands = loadDoubles("rands.dat");
    if (rands.size() != p.nTimeSteps * p.nRuns * p.N)
    {
        std::cerr << "Invalid rands length." << '\n';
        return -1;
    }
    ...
    {
        results.push_back(
            pool.enqueue(evolve, rands.data() + (run * p.nTimeSteps * p.N), H0, Hf0, c0, xxsq, p));
    }
